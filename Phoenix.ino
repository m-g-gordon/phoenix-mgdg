...
INCONTROLSTATE   g_ControlState;      // This is our global Input control state object...

// Define our ServoWriter class
ServoDriver  g_ServoDriver;      // our global servo driver class

// Define Pan and Tilt servo controller
#ifndef PT_ON_SSC32
MicroMaestro g_PTServoDriver(MAESTROSerial, 255, 12);
#endif

//Define our current Video Transmitter state
boolean g_VideoTRXOn;

//Define our current Headlights state
boolean g_HeadlightsOn;

boolean         g_fLowVoltageShutdown;    // If set the bot shuts down because the input voltage is to low

//--boolean         g_ControlState.fHexOn;            //Switch to turn on Phoenix
//--boolean         g_ControlState.fPrev_HexOn;        //Previous loop state
//--------------------------------------------------------------------
//[Balance]
long            TotalTransX;
long            TotalTransZ;
...
extern boolean g_fShowDebugPrompt;
...
void setup() {
  ...
  g_ControlState.SelectedLeg = 255; // No Leg selected
  PrevSelectedLeg = 255;
  ...
  //Gait
  g_ControlState.GaitType = 1;  // 0; Devon wanted
  g_ControlState.BalanceMode = 0;
...
void loop(void)
{
  ...
  // Do Pan Tilt Outputs
  //Pan Right
  if (g_ControlState.PT_PAD_RIGHT) {
    ptposition_x -= 20;
    if (ptposition_x < 800) {
      ptposition_x = 800;
    }
  ...
  //Pan Left
  if (g_ControlState.PT_PAD_LEFT) {
    ptposition_x += 20;
    if (ptposition_x > 2100) {
      ptposition_x = 2100;
    }
  ...
  //Tilt Down
  if (g_ControlState.PT_PAD_DOWN) {
    ptposition_y += 20;
    if (ptposition_y > 2100)
    {
      ptposition_y = 2100;
    }
  ...
  //Video Camera State
  if (g_ControlState.VIDEO_TRX )  {
    if(!g_VideoTRXOn){
      digitalWrite(VIDEOTRANSMIT, HIGH);
      g_VideoTRXOn = true;
    }
  }
  else {
  ...
  for (LegIndex = 0; LegIndex <= 2; LegIndex++) {
    BodyFK(-LegPosX[LegIndex] + g_ControlState.BodyPos.x + GaitPosX[LegIndex] - TotalTransX,
           LegPosZ[LegIndex] + g_ControlState.BodyPos.z + GaitPosZ[LegIndex] - TotalTransZ,
           LegPosY[LegIndex] + g_ControlState.BodyPos.y + GaitPosY[LegIndex] - TotalTransY,
           GaitRotY[LegIndex], LegIndex);

    LegIK (LegPosX[LegIndex] - g_ControlState.BodyPos.x + BodyFKPosX - (GaitPosX[LegIndex] - TotalTransX),
           LegPosY[LegIndex] + g_ControlState.BodyPos.y - BodyFKPosY + GaitPosY[LegIndex] - TotalTransY,
           LegPosZ[LegIndex] + g_ControlState.BodyPos.z - BodyFKPosZ + GaitPosZ[LegIndex] - TotalTransZ, LegIndex);
  }
  ...
  if (g_ControlState.fHexOn) {
    if (g_ControlState.fHexOn && !g_ControlState.fPrev_HexOn)
    {
    ...
    if ((abs(g_ControlState.TravelLength.x) > cTravelDeadZone) || (abs(g_ControlState.TravelLength.z) > cTravelDeadZone) ||
      ...
      ServoMoveTime = NomGaitSpeed + (g_ControlState.InputTimeDelay * 2) + g_ControlState.SpeedControl;

      //Add aditional delay when Balance mode is on
      if (g_ControlState.BalanceMode)
        ServoMoveTime = ServoMoveTime + 100;
    } else //Movement speed excl. Walking
      ServoMoveTime = 200 + g_ControlState.SpeedControl;
  ...
  {
    //Turn the bot off
    if (g_ControlState.fPrev_HexOn || (AllDown = 0)) {
      ServoMoveTime = 600;
      StartUpdateServos();
  ...
  //Store previous g_ControlState.fHexOn State
  if (g_ControlState.fHexOn)
    g_ControlState.fPrev_HexOn = 1;
  else
    g_ControlState.fPrev_HexOn = 0;
}
  ...
  if (g_ControlState.SelectedLeg <= 5) 
  {
    if (g_ControlState.SelectedLeg != PrevSelectedLeg) 
    {
      ...
      { //Lift leg a bit when it got selected
        ...
        PrevSelectedLeg = g_ControlState.SelectedLeg;
      } 
      else 
      {
    ...
    else if (!g_ControlState.fSLHold) 
    {
      LegPosY[g_ControlState.SelectedLeg] = LegPosY[g_ControlState.SelectedLeg] + g_ControlState.SLLeg.y;
      LegPosX[g_ControlState.SelectedLeg] = (short)pgm_read_word(&cInitPosX[g_ControlState.SelectedLeg]) + g_ControlState.SLLeg.x;
      LegPosZ[g_ControlState.SelectedLeg] = (short)pgm_read_word(&cInitPosZ[g_ControlState.SelectedLeg]) + g_ControlState.SLLeg.z;
    }
  } 
  else 
  {
...
{
  //Gait selector
  switch (g_ControlState.GaitType)  {
    case 0:
...
void GaitSeq(void)
{
  ...
  TravelRequest = ((abs(g_ControlState.TravelLength.x) > cTravelDeadZone) || (abs(g_ControlState.TravelLength.z) > cTravelDeadZone) || (abs(g_ControlState.TravelLength.y) > cTravelDeadZone));
  if (NrLiftedPos == 5)
    LiftDivFactor = 4;
  else
    LiftDivFactor = 2;
...
void Gait (byte GaitCurrentLegNr)
{
  ...
  if ((TravelRequest && (NrLiftedPos == 1 || NrLiftedPos == 3 || NrLiftedPos == 5) &&
    ...
    GaitPosY[GaitCurrentLegNr] = -g_ControlState.LegLiftHeight;
  ...
  //Optional Half heigth Rear (2, 3, 5 lifted positions)
           ...
           && TravelRequest) {
    GaitPosX[GaitCurrentLegNr] = -g_ControlState.TravelLength.x / LiftDivFactor;
    GaitPosY[GaitCurrentLegNr] = -3 * g_ControlState.LegLiftHeight / (3 + HalfLiftHeigth); //Easier to shift between div factor: /1 (3/3), /2 (3/6) and 3/4
    GaitPosZ[GaitCurrentLegNr] = -g_ControlState.TravelLength.z / LiftDivFactor;
    GaitRotY[GaitCurrentLegNr] = -g_ControlState.TravelLength.y / LiftDivFactor;
  }
  ...
  //Optional Half heigth Rear 5 LiftedPos (5 lifted positions)
    ...
    GaitPosX[GaitCurrentLegNr] = -g_ControlState.TravelLength.x / 2;
    GaitPosY[GaitCurrentLegNr] = -g_ControlState.LegLiftHeight / 2;
    GaitPosZ[GaitCurrentLegNr] = -g_ControlState.TravelLength.z / 2;
    GaitRotY[GaitCurrentLegNr] = -g_ControlState.TravelLength.y / 2;
  }
  ...
  //Move body forward
    ...
    GaitPosX[GaitCurrentLegNr] = GaitPosX[GaitCurrentLegNr] - (g_ControlState.TravelLength.x / TLDivFactor);
    GaitPosY[GaitCurrentLegNr] = 0;
    GaitPosZ[GaitCurrentLegNr] = GaitPosZ[GaitCurrentLegNr] - (g_ControlState.TravelLength.z / TLDivFactor);
    GaitRotY[GaitCurrentLegNr] = GaitRotY[GaitCurrentLegNr] - (g_ControlState.TravelLength.y / TLDivFactor);
  }
...
void BalCalcOneLeg (short PosX, short PosZ, short PosY, byte BalLegNr)
{
  ...
  CPR_Y = 150 + PosY;        // using the value 150 to lower the centerpoint of rotation 'g_ControlState.BodyPos.y +
...
void BodyFK (short PosX, short PosZ, short PosY, short RotationY, byte BodyIKLeg)
{
  ...
  GetSinCos (g_ControlState.BodyRot1.x + TotalXBal1);
  SinG4 = sin4;
  CosG4 = cos4;

  GetSinCos (g_ControlState.BodyRot1.z + TotalZBal1);
  SinB4 = sin4;
  CosB4 = cos4;

  GetSinCos (g_ControlState.BodyRot1.y + (RotationY * c1DEC) + TotalYBal1) ;
  SinA4 = sin4;
  CosA4 = cos4;
