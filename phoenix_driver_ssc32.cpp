...
//[Report Version]
//--------------------------------------------------------------------
void ServoDriver::ReportVersion()
{
  char buf[40];        // give a nice large buffer.
  for (int i = 0; i < 40; i++) buf[i] = 0;
  byte cbRead;

  SSCSerial.print("ver\r");
  cbRead = SSCRead((byte*)buf, sizeof(buf), 10000, 13);
  ...
  if (cbRead > 0)
  {
    ...
    String verinfo = buf;
    DBGSerial.println(verinfo);
  }
  else
  {
    DBGSerial.println("No Version Info");
  }
}
//--------------------------------------------------------------------
//[GP PLAYER]
...
#ifdef PT_ON_SSC32
//------------------------------------------------------------------------------------------
//[UpdatePanPosition()] Manages Pan Servo Commands
//------------------------------------------------------------------------------------------
void ServoDriver::UpdatePanPosition(int channel, int pos_x)    
{
    SSCSerial.print("#");
    SSCSerial.print(channel, DEC);
    SSCSerial.print("P");
    SSCSerial.print(pos_x, DEC);
    SSCSerial.println("S900");
    //delay(100);
}


//------------------------------------------------------------------------------------------
//[UpdateTiltPosition()] Manages Tilt Servo Commands
//------------------------------------------------------------------------------------------
void ServoDriver::UpdateTiltPosition(int channel, int pos_y)    // Start the update
{
  SSCSerial.print("#");
  SSCSerial.print(channel, DEC);
  SSCSerial.print("P");
  SSCSerial.print(pos_y, DEC);
  SSCSerial.println("S900");
  //delay(100);
}
